...
mysql_select_db($database_innebandybase, $innebandybase);
$query_PlayerInfo = "SELECT players.firstName, players.lastName, players.phone, attention.notes, attentionType.type FROM (players LEFT JOIN attention ON players.id = attention.player) LEFT JOIN attentionType ON attention.type = attentionType.id WHERE (((attention.event)=".$row_NextEvent['id'].")) ORDER BY attentionType.value DESC;";
...
mysql_select_db($database_innebandybase, $innebandybase);
$query_attentionsNext = "SELECT attention.type, Count(attention.id) AS countAttentions, events.id
...
echo '<card id="defaultId" title="'.$row_NextEvent['description'].'" xml:lang="DefaultXML" newcontext="false">';
...
echo '<br/>'.$row_NextEvent['location'].'<br/>Antall bekreftede: ';
echo $row_confirmed['confirmedCount'] ? $row_confirmed['confirmedCount'] : '0';
echo '</p><p>'.$row_NextEvent['notes'].'</p><p><strong>Oppsummering</strong><br/>';
